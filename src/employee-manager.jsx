            ...
            <StatCard
              ...
              value={
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                  ...
                  <span style={{ display: 'flex', flexDirection: 'row', gap: '2px', alignItems: 'center', flexShrink: 0 }}>
                    <span className="text-[9px] font-bold rounded border flex gap-1" style={{ padding: '1px 4px', backgroundColor: '#FFF7ED', borderColor: '#FED7AA', color: stats.projectExt > 0 ? '#C2410C' : '#FDBA74', opacity: stats.projectExt > 0 ? 1 : 0.45 }}>
                      <span>대외</span><span>{stats.projectExt}</span>
                    </span>
                    <span className="text-[9px] font-bold rounded border flex gap-1" style={{ padding: '1px 4px', backgroundColor: '#EFF6FF', borderColor: '#BFDBFE', color: stats.projectBank > 0 ? '#1D4ED8' : '#93C5FD', opacity: stats.projectBank > 0 ? 1 : 0.45 }}>
                      <span>행내</span><span>{stats.projectBank}</span>
                    </span>
                    <span className="text-[9px] font-bold rounded border flex gap-1" style={{ padding: '1px 4px', backgroundColor: '#F0FDF4', borderColor: '#BBF7D0', color: stats.projectInt > 0 ? '#15803D' : '#86EFAC', opacity: stats.projectInt > 0 ? 1 : 0.45 }}>
            ...
            <StatCard
                ...
                value={
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                    ...
                    <span style={{ display: 'flex', flexDirection: 'row', gap: '2px', alignItems: 'center', flexShrink: 0 }}>
                      <span className="text-[9px] font-bold rounded border flex gap-1" style={{ padding: '1px 4px', backgroundColor: '#EFF6FF', borderColor: '#BFDBFE', color: stats.waitingEmp > 0 ? '#1D4ED8' : '#93C5FD', opacity: stats.waitingEmp > 0 ? 1 : 0.45 }}>
                        <span>직원</span><span>{stats.waitingEmp}</span>
                      </span>
                      <span className="text-[9px] font-bold rounded border flex gap-1" style={{ padding: '1px 4px', backgroundColor: '#F5F3FF', borderColor: '#DDD6FE', color: stats.waitingProf > 0 ? '#6D28D9' : '#C4B5FD', opacity: stats.waitingProf > 0 ? 1 : 0.45 }}>
...
function StatCard({ icon, label, value, accent }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-2 sm:p-3 flex items-center gap-2 min-w-0" style={{ height: '60px', boxSizing: 'border-box' }}>
